...
class Switch:
    ...
    #-----------------------------------------------------------------------
    @classmethod
    def parse_json(cls, data, have_mode=True):
        """Parse on/off JSON data from an input message payload.

        The on/off flag is controlled by the data['cmd'] attribute which must
        be 'on' or 'off'.

        The on/off mode is NORMAL by default.  It can be set by the optional
        field data['mode'] which can be 'normal', 'fast', or 'instant'.  Or
        it can be set by the boolean fields data['fast'] or data['instant'].
        ...
        Returns:
          (bool is_on, on_off.Mode): Returns a boolean to indicate on/off and
          the requested on/off mode enumeration to use.
        """
