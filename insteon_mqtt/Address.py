...
class Address:
    ...
    constructed from a variety of inputs (see the constructor) and
    supports conversion to/from bytes (for I/O) and json (for
    configuration files).
    ...
    The Address class supports hash and comparisons so it can be used
    as a dictionary key.
    """
    #-----------------------------------------------------------------------
    @staticmethod
    ...
    def __init__(self, addr, addr2=None, addr3=None):
        ...
        An address has three bytes AA, BB, and CC that need to be
        input.  The address can be input as a single field or as three
        fields.  Valid single field inputs are:

        - String containing the hex address in upper or lower case.
          Valid inputs have the form 'AABBCC', 'AA.BB.CC', 'AA:BB:CC',
          or 'AA BB CC'
        ...
        - Strings with a single hex byte input in each.  Valid forms
          are 'A0' or '0xA0'
