...
class OutStandard(Base):
    ...
    When sending, this will be 8 bytes long.  When receiving back from the
    modem, it will be 9 bytes (8+ack/nak).  The from_bytes() function can
    also return an OutExtended if the extended message bit is set.

    The response from the modem to this message will depend on the cmd1/cmd2
    command field inputs.
    """
    msg_code = 0x62
    fixed_msg_size = 9
...
class OutExtended(OutStandard):
    ...
    When sending, this will be 22 bytes long.  When receiving back from the
    modem, it will be 23 bytes (22+ack/nak).  Since this hsa the same message
    code as OutStandard, use OutStandard.from_bytes() to read either type.
    ...
    def direct(cls, to_addr, cmd1, cmd2, data, crc_type="D14"):
        ...
        Args:
          ...
          crc_type (str):   None, "D14", or "CRC". See explanation in
                   __init__().
    ...
    def __init__(self, to_addr, flags, cmd1, cmd2, data, is_ack=None,
        ...
        computed and set into the last (D14) byte.  The Insteon developer
        docs don't say this and say that bytes unused but it is required.
        Valid inputs are:

        - "D14": Single byte checksum used by device database modification
          messages.
        - "CRC": 2 byte CRC (D13, D14) used by thermostat commands.

        Args:
          to_addr (Address):  The adddress to send the command to.
          flags (Flags):  Message flags to use.
          cmd1 (int):  The command 1 field to set.
          cmd2 (int):  The command 2 field to set.
          data (byte):  The extended data array of 14 bytes.
    ...
    def to_bytes(self):
        ...
        Returns:
          bytes:  Returns the message as bytes.
        """
        # NOTE: both of these checksum/CRC algorithms were built from the
        # insteon-terminal messages.py file at:
        # https://github.com/pfrommerd/insteon-terminal

        # Even though the Insteon docs say that the last byte is unused, db
        # modification commands (cmd1=0x2f) require that a checksum be
        # computed and put in the last byte.
