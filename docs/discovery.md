...
HomeAssistant allows for Insteon-MQTT to define entities using a discovery
protocol. This means, that for general installations, a user need only setup
Insteon-MQTT following the
...
The base configuration file that ships inside Insteon-MQTT contains the initial templates for all Insteon devices.

> __If you installed Insteon-MQTT starting with version 0.8.3 or earlier__, you will
...
If the default entities defined by Insteon-MQTT do not suit your needs, you may
...
It may also be the case that Insteon-MQTT has defined a number of entities that
...
Insteon-MQTT `yaml` config file.  Then restart Insteon-MQTT.  You then need to
...
disabled, then you have not 1) removed the device from your Insteon-MQTT config,
2) restarted Insteon-MQTT, OR 3) restared HomeAssistant.
...
is first ingested from YAML.  You can read about
...
template as a value, it will be rendered by Insteon-MQTT, which in this case
  ...
  kpl_6:  # < Note the class name
    discovery_entities:
      - component: 'light'
       ...
       # No button 2 on 6-button devices
      - component: 'switch'
        config: |-
          {
            "uniq_id": "{{address}}_3",
...
Then for each device specify that discovery class:
