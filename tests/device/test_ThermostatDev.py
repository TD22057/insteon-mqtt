...
def test_device(tmpdir):
    ...
    device = Thermo(protocol, modem, addr)
    return device
...
class Test_Thermostat:
    ...
    @pytest.mark.parametrize("group_num,cmd1,cmd2,expected", [
        (0x01,Msg.CmdType.ON, 0x00,[Thermo.Status.COOLING]),
        (0x01,Msg.CmdType.OFF, 0x00, [Thermo.Status.OFF]),
        (0x01,Msg.CmdType.LINK_CLEANUP_REPORT, 0x00, None),
        (0x02,Msg.CmdType.ON, 0x00,[Thermo.Status.HEATING]),
        (0x02,Msg.CmdType.OFF, 0x00, [Thermo.Status.OFF]),
        # Humid not currently handled
        (0x03,Msg.CmdType.ON, 0x00, None),
        (0x04,Msg.CmdType.ON, 0x00, None),
        (0xEF,Msg.CmdType.ON, 0x00,None),
    ])
